    ...
    public static void update() {
        ...
        if (overlayMessage != null) {
            ...
            if (FaceFont.keys(FType.HURT_TIME).stream().anyMatch(overlayText::contains))
                xyz.mashtoolz.config.FaceConfig.General.lastHurtTime = System.currentTimeMillis();
        }

        assert INSTANCE.CLIENT.player != null;
        int playerHurtTime = INSTANCE.CLIENT.player.hurtTime;
        if (xyz.mashtoolz.config.FaceConfig.General.hurtTime == 0 && playerHurtTime != 0)
            xyz.mashtoolz.config.FaceConfig.General.hurtTime = playerHurtTime;

        if (xyz.mashtoolz.config.FaceConfig.General.hurtTime == -1 && playerHurtTime == 0)
            xyz.mashtoolz.config.FaceConfig.General.hurtTime = 0;

        if (xyz.mashtoolz.config.FaceConfig.General.hurtTime > 0) {
            xyz.mashtoolz.config.FaceConfig.General.hurtTime = -1;
